...
#### 템플릿 파일과 시험지 이미지 파일 경로 설정
mark_template_path = '/Users/choeyuhyeon/Desktop/auto_score/시험지/SampleData/mark_template'
original_path = '/Users/choeyuhyeon/Desktop/auto_score/시험지/SampleData/MCQ/T2/Answer'
answer_path = '/Users/choeyuhyeon/Desktop/auto_score/시험지/SampleData/MCQ/T2/Answer'
scanned_path = '/Users/choeyuhyeon/Desktop/auto_score/시험지/SampleData/MCQ/T2/Scanned'
    ...
    if len(answer_loc[page]) == len(scn_mark_loc):
        ...
        for a,b in zip(answer_loc[page],scn_mark_loc):
            ...
            if distance(a, b) < 15 : 
                isCorrect = True
                point = 5
                print(f"{i}번: 정답 [{point}]")
            else : 
                print(f"{i}번: 오답 [{point}]")
            i += 1
        ...
        print(f"File #{scn_num} \nStudent Info : \n\tName : {studentName} \n\tSerial : {studentSerial} \n\tPage : {page+1}")
        print(f"ERR: File #{scn_num}={page+1}p 마킹갯수가 맞지 않습니다 {len(scn_mark_loc)} of {len(answer_loc[page])}")
    print("=======================")
